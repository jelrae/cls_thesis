% this script is similar to Fig6_ACCmultiplesignals, but to address reviewer #3's comment,
% we now do the subspace projects across 100 random subsamples of the
% trials
clear all; close all;
[bd] = get_basedir;
...
%% ALTERNATIVE: the same thing for 'random projections' of data (requested by reviewer 3)

load(fullfile(bd,'neuronal_regression_results','info_gathering_ramp_to_choice.mat'),'av_Cue1L_proj');

for i = 1:100
   dm_proj = [demean(subspace_belief_proj(area==aoi,i)) demean(subspace_RL_proj(area==aoi,i)) ...
       ones(size(subspace_belief_proj(area==aoi,i))) (subspace_A_proj(area==aoi,i)+subspace_O_proj(area==aoi,i))/2];
   data = av_Cue1L_proj(area==aoi,:,:,i);
   datr = squeeze(data(:,:));
   tmp = ols(datr,dm_proj,eye(nSub));
   cc_proj(:,:,:,i) = reshape(tmp,[nSub size(data,2) size(data,3)]);
end

%average these random projections:
cc_stim = (mean(cc_proj,4));

clear av_Cue1L_proj %as these projections eat RAM
...
load(fullfile(bd,'neuronal_regression_results','info_gathering_ramp_to_choice.mat'),'av_Cue1RT_LR_proj');

for i = 1:100
   dm_proj = [demean(subspace_belief_proj(area==aoi,i)) demean(subspace_RL_proj(area==aoi,i)) ...
       ones(size(subspace_belief_proj(area==aoi,i))) (subspace_A_proj(area==aoi,i)+subspace_O_proj(area==aoi,i))/2];
   data = av_Cue1RT_LR_proj(area==aoi,:,:,:,i);
   datr = squeeze(data(:,:));
   badcells = any(isnan(datr),2); %a few cells contain nans due to lack of appropriate trials - trim these
   tmp = ols(datr(~badcells,:),dm_proj(~badcells,:),eye(nSub));
   ccLR_proj(:,:,:,:,i) = reshape(tmp,[nSub size(data,2) size(data,3) size(data,4)]);
end

%average these random projections:
ccLR_stim = (mean(ccLR_proj,5));

clear av_Cue1RT_LR_proj %as these projections eat RAM
...
load(fullfile(bd,'neuronal_regression_results','info_gathering_ramp_to_choice.mat'),'av_RespRT_LR_proj');

for i = 1:100
    dm_proj = [demean(subspace_belief_proj(area==aoi,i)) demean(subspace_RL_proj(area==aoi,i)) ...
        ones(size(subspace_belief_proj(area==aoi,i))) (subspace_A_proj(area==aoi,i)+subspace_O_proj(area==aoi,i))/2];
    data = av_RespRT_LR_proj(area==aoi,:,:,:,i);
    datr = squeeze(data(:,:));
    badcells = any(isnan(datr),2); %a few cells contain nans due to lack of appropriate trials - trim these
    tmp = ols(datr(~badcells,:),dm_proj(~badcells,:),eye(nSub));
    ccLR_resp_proj(:,:,:,:,i) = reshape(tmp,[nSub size(data,2) size(data,3) size(data,4)]);
end

ccLR_resp = mean(ccLR_resp_proj,5);
clear av_RespRT_LR_proj %as these projections eat RAM
