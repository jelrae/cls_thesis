...
%            % The shape the data needs to be in for it to work well.    
%             test = reshape(trial_lfp_img1,[size(trial_lfp_img1,1),1,size(trial_lfp_img1,2)]);
%             test(:,end+1,:) = reshape(trial_lfp_img2,[size(trial_lfp_img2,1),1,size(trial_lfp_img2,2)]);

            test_conc = cat(1, trial_lfp_img1, trial_lfp_img2);
            test_conc = cat(3, trial_lfp_img1, trial_lfp_img2);
            test_reshape = reshape(trial_lfp_img1, [size(trial_lfp_img1,1), 1, size(trial_lfp_img1,2)]);
            test = cell(1,450);
            test_reshape_2 = reshape(trial_lfp_img2, [size(trial_lfp_img2,1), 1, size(trial_lfp_img2,2)]);
            test_conc = cat(2, test_reshape, test_reshape_2);
            %% Testing things for making it in fieldtrip
            data.label = {'15'};
            data.fsample = 1000;
            time = 0:(300-1);
